...
def calweight(dataset):
    num = {}
    for data, label in dataset:
        if label not in num.keys():
            num[label] = 0
        num[label] += 1
    print('sample number in each class:', num)
    max_num = 0
    for key in num.keys():
        if num[key] > max_num:
            max_num = num[key]
    print('max_sample_num:', max_num)
    w_cls = {}
    for key in num.keys():
        w_cls[key] = round(max_num/num[key], 2)
    print('weight for each class', w_cls)
    weight = []
    for data, label in dataset:
        weight.append(w_cls[label])
    return weight


def getdataloader(dataset, batchsize, num_workers=4, drop_last=True, weightsampler=True):
    if weightsampler:
        print('Sample data by weight')
        weight = calweight(dataset)
        sampler = WeightedRandomSampler(weight, num_samples=len(dataset), replacement=True)
        dataloader = DataLoader(dataset, batch_size=batchsize, sampler=sampler, num_workers=num_workers, drop_last=drop_last)
    else:
        dataloader = DataLoader(dataset, batch_size=batchsize, shuffle=True, num_workers=num_workers, drop_last=drop_last)
    return dataloader
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='domain-invariant and class-distinguishable domain adaptation network')
    parser.add_argument('--method', type=str, default='proposed', choices=['CDAN', 'CDAN+E', 'MADA','DANN', 'proposed', 'base'])
    ...
    parser.add_argument('--dset', type=str, default='visda', choices=['office', 'image-clef', 'visda', 'office-home', 'digit'],
                        help="The dataset or source dataset used")
    parser.add_argument('--s_dset_path', type=str, default='../data/visda/train',
                        help="The source dataset path list")
    parser.add_argument('--t_dset_path', type=str, default='../data/visda/validation',
    ...
    parser.add_argument('--tsne_num', type=int, default=100, help='the times to carry out tsne')
    parser.add_argument('--trade_off23', type=float, default=0.03, help='the value of lambda 2 and lambda 3')
    ...
    config["output_path"] = "snapshot/" + args.method + '-' +args.s_dset_path.split('/')[-1] + '-' + args.t_dset_path.split('/')[-1] + '-lr-' + str(args.lr)
    config['center_threshold'] = args.center_threshold
    config['tsne'] = args.tsne
    config['tsne_num'] = args.tsne_num
    if not osp.exists(config["output_path"]):
        os.system('mkdir -p ' + config["output_path"])
    config["out_file"] = open(osp.join(config["output_path"],
                                       'log_'+args.s_dset_path.split('/')[-1]+'-'+args.t_dset_path.split('/')[-1]+'-'+str(args.trade_off23)+'.txt')
                              , "w")
    config['A_distance_file'] = open(osp.join(config["output_path"],
                                              'A_distance_file_'+args.s_dset_path.split('/')[-1]+'-'+args.t_dset_path.split('/')[-1]+'-'+str(args.trade_off23)+'.txt')
                                     , 'w')

    config["prep"] = {"test_10crop": False, 'params': {"resize_size": 256, "crop_size": 224, 'alexnet': False}}
    config["loss"] = {"trade_off": 1.0}
    config["loss"]['trade_off23'] = args.trade_off23
